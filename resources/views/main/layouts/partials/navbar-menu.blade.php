<ul class="list-reset flex flex-col lg:flex-row justify-center lg:justify-end flex-1 items-center">

    <!--Hoverable Link-->
    <li class="hoverable menu--border hidden lg:block">
        <a href="{{route('page', 'services')}}"
           class="inline-block relative py-2 px-2 lg:p-5 text-sm lg:text-base font-bold">{{__('menu.services')}}</a>
        <div class="p-6 mega-menu mb-16 sm:mb-0 shadow-xl gradient border-t-2">
            <div class="container mx-auto w-full flex flex-wrap justify-between mx-2">
                {{-- <div class="w-full text-white mb-8">
                  <h2 class="font-bold text-2xl">Main Hero Message for the menu section</h2>
                  <p>Sub-hero message, not too long and not too short. Make it just right!</p>
                </div> --}}
                <ul class="px-4 w-full sm:w-1/2 lg:w-1/3 pb-6 pt-6 lg:pt-3">
                    <div class="mb-2">
                        <a href="#" class="flex items-center">
                            <svg class="h-5 mb-3 mr-4 fill-current text-white"
                                 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M0 9h24v6h-24z"/>
                            </svg>
                            <p class="font-bold text-base text-white mb-2 menu--border">{{__('menu.service-video')}}</p>
                        </a>
                    </div>
                    <div class="mb-2">
                        <a href="#" class="flex items-center">
                            <svg class="h-5 mb-3 mr-4 fill-current text-white"
                                 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M0 9h24v6h-24z"/>
                            </svg>
                            <p class="font-bold text-base text-white mb-2 menu--border">{{__('menu.service-skd')}}</p>
                        </a>
                    </div>
                    <div class="mb-2">
                        <a href="#" class="flex items-center">
                            <svg class="h-5 mb-3 mr-4 fill-current text-white"
                                 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M0 9h24v6h-24z"/>
                            </svg>
                            <p class="font-bold text-base text-white mb-2 menu--border">{{__('menu.service-secur')}}</p>
                        </a>
                    </div>
                    <div class="mb-2">
                        <a href="#" class="flex items-center">
                            <svg class="h-5 mb-3 mr-4 fill-current text-white"
                                 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M0 9h24v6h-24z"/>
                            </svg>
                            <p class="font-bold text-base text-white mb-2 menu--border">{{__('menu.service-ksb')}}</p>
                        </a>
                    </div>
                    <div class="mb-2">
                        <a href="#" class="flex items-center">
                            <svg class="h-5 mb-3 mr-4 fill-current text-white"
                                 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M0 9h24v6h-24z"/>
                            </svg>
                            <p class="font-bold text-base text-white mb-2 menu--border">{{__('menu.service-biometr')}}</p>
                        </a>
                    </div>
                </ul>
                <ul class="px-4 w-full sm:w-1/2 lg:w-1/3 pb-6 pt-6 lg:pt-3">
                    <div class="mb-2">
                        <a href="#" class="flex items-center">
                            <svg class="h-5 mb-3 mr-4 fill-current text-white"
                                 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M0 9h24v6h-24z"/>
                            </svg>
                            <p class="font-bold text-base text-white mb-2 menu--border">{{__('menu.service-lowcur')}}</p>
                        </a>
                    </div>
                    <div class="mb-2">
                        <a href="#" class="flex items-center">
                            <svg class="h-5 mb-3 mr-4 fill-current text-white"
                                 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M0 9h24v6h-24z"/>
                            </svg>
                            <p class="font-bold text-base text-white mb-2 menu--border">{{__('menu.service-sks')}}</p>
                        </a>
                    </div>
                    <div class="mb-2">
                        <a href="#" class="flex items-center">
                            <svg class="h-5 mb-3 mr-4 fill-current text-white"
                                 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M0 9h24v6h-24z"/>
                            </svg>
                            <p class="font-bold text-base text-white mb-2 menu--border">{{__('menu.service-domohhone')}}</p>
                        </a>
                    </div>
                    <div class="mb-2">
                        <a href="#" class="flex items-center">
                            <svg class="h-5 mb-3 mr-4 fill-current text-white"
                                 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M0 9h24v6h-24z"/>
                            </svg>
                            <p class="font-bold text-base text-white mb-2 menu--border">{{__('menu.service-dispatching')}}</p>
                        </a>
                    </div>
                    <div class="mb-2">
                        <a href="#" class="flex items-center">
                            <svg class="h-5 mb-3 mr-4 fill-current text-white"
                                 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M0 9h24v6h-24z"/>
                            </svg>
                            <p class="font-bold text-base text-white mb-2 menu--border">{{__('menu.service-projects')}}</p>
                        </a>
                    </div>
                </ul>
                <ul class="px-4 w-full sm:w-1/2 lg:w-1/3 border-gray-600 border-b sm:border-b-0 md:border-b-0 pb-6 pt-6 lg:pt-3">
                    <div class="mb-2">
                        <a href="#" class="flex items-center">
                            <svg class="h-5 mb-3 mr-4 fill-current text-white"
                                 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M0 9h24v6h-24z"/>
                            </svg>
                            <p class="font-bold text-base text-white mb-2 menu--border">{{__('menu.service-multimedia')}}</p>
                        </a>
                    </div>
                    <div class="mb-2">
                        <a href="#" class="flex items-center">
                            <svg class="h-5 mb-3 mr-4 fill-current text-white"
                                 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M0 9h24v6h-24z"/>
                            </svg>
                            <p class="font-bold text-base text-white mb-2 menu--border">{{__('menu.service-conf')}}</p>
                        </a>
                    </div>
                    <div class="mb-2">
                        <a href="#" class="flex items-center">
                            <svg class="h-5 mb-3 mr-4 fill-current text-white"
                                 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M0 9h24v6h-24z"/>
                            </svg>
                            <p class="font-bold text-base text-white mb-2 menu--border">{{__('menu.service-lvs')}}</p>
                        </a>
                    </div>
                    <div class="mb-2">
                        <a href="#" class="flex items-center">
                            <svg class="h-5 mb-3 mr-4 fill-current text-white"
                                 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M0 9h24v6h-24z"/>
                            </svg>
                            <p class="font-bold text-base text-white mb-2 menu--border">{{__('menu.service-vols')}}</p>
                        </a>
                    </div>
                    <div class="mb-2">
                        <a href="#" class="flex items-center">
                            <svg class="h-5 mb-3 mr-4 fill-current text-white"
                                 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M0 9h24v6h-24z"/>
                            </svg>
                            <p class="font-bold text-base text-white mb-2 menu--border">{{__('menu.service-parking')}}</p>
                        </a>
                    </div>
                </ul>
            </div>
        </div>
    </li>

    <li class="hoverable menu--border hidden lg:block">
        <a href="#"
           class="inline-block relative py-2 px-2 lg:p-5 text-sm lg:text-base font-bold">{{__('menu.equipment')}}</a>
        <div class="p-6 mega-menu mb-16 sm:mb-0 shadow-xl gradient border-t-2">
            <div class="container mx-auto w-full flex flex-wrap justify-between mx-2">
                {{-- <div class="w-full text-white mb-8">
                  <h2 class="font-bold text-2xl">Main Hero Message for the menu section</h2>
                  <p>Sub-hero message, not too long and not too short. Make it just right!</p>
                </div> --}}
                <ul class="px-4 w-full sm:w-1/2 lg:w-1/3 pb-6 pt-6 lg:pt-3">
                    <div class="mb-2">
                        <a href="{{ route('page', 'intercom-systems') }}" class="flex items-center">
                            <svg class="h-5 mb-3 mr-4 fill-current text-white"
                                 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M0 9h24v6h-24z"/>
                            </svg>
                            <p class="font-bold text-base text-white mb-2 menu--border">BAS-IP</p>
                        </a>
                    </div>
                </ul>
                <ul class="px-4 w-full sm:w-1/2 lg:w-1/3 pb-6 pt-6 lg:pt-3">
                    <div class="mb-2">
                        <a href="{{ route('page', 'domofonnye-sistemy') }}" class="flex items-center">
                            <svg class="h-5 mb-3 mr-4 fill-current text-white"
                                 xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path d="M0 9h24v6h-24z"/>
                            </svg>
                            <p class="font-bold text-base text-white mb-2 menu--border">HIKVISION</p>
                        </a>
                    </div>
                </ul>
            </div>
        </div>
    </li>

    <li class="menu--border block lg:hidden">
        <a href="{{route('page', 'services')}}"
           class="inline-block relative py-2 px-2 lg:p-5 text-sm lg:text-base font-bold">{{__('menu.services')}}</a>
    </li>

    <li class="menu--border">
        <a href="{{route('portfolio')}}"
           class="inline-block relative py-2 px-2 lg:p-5 text-sm lg:text-base font-bold">{{__('menu.portfolio')}}</a>
    </li>
    <li class="menu--border">
        <a href="{{route('page', 'about-us')}}"
           class="inline-block relative py-2 px-2 lg:p-5 text-sm lg:text-base font-bold">{{__('menu.ab-us')}}</a>
    </li>
    <li class="menu--border">
        <a href="{{route('contacts')}}"
           class="inline-block relative py-2 px-2 lg:p-5 text-sm lg:text-base font-bold">{{__('menu.contacts')}}</a>
    </li>

</ul>
